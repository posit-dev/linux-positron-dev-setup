...
install_python() {
  ...
  # environment" for Fedora/RHEL). Install them only when some are missing.
  local build_deps=(
...
install_vscode() {
  ...
  # curl fetches the .rpm over HTTPS; make sure it's present.
  if ! pkg_installed curl; then
    log "installing curl..."
    sudo dnf install -y curl
  fi
...
# and `systemctl enable --now` is safe to re-run.
install_ssh_server() {
